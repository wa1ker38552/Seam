...
def run_iteration(message):
    ...
    sender_score = -20 if ('no' in message['From'].lower() and 'reply' in message['From'].lower()) or 'update' in message['From'].lower() or 'info' in message['From'].lower() or 'notification' in message['From'].lower() or 'support' in message['From'].lower() else 0
    ...
    try:
        ...
        trustworthy_score = int(prompt_deepinfra('Given a name rate how trustworthy the name sounds. For example, companies or things that do not sound like real names should have less score. Names that sound like its from a real person should be given a higher score. Return ONLY A NUMBER from 0-5 without ANY ADDITIONAL NOTES', message['From'].split('<')[0].strip()))
        domain_score = int(prompt_deepinfra('Given a domain, rate how trustworthy it is. For exmaple, gmail should be a 10 while abc.123.com should be given a lower score. Return ONLY A NUMBER from 0-5 without ANY ADDITIONAL NOTES', message['From'].split('@')[1].split('>')[0]))
...
THREADS = 4 # one for each api key
thread_list = []
